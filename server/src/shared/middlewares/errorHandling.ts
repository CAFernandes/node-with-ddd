...
export const errorHandling = (err: Error, _: Request, res: Response, __: NextFunction) => {
  ...
  if (err instanceof UnauthorizedError) return res.status(err.code).json({ status: 'error', message: err.message })
  if (err instanceof BadRequest) return res.status(err.code).json({ status: 'error', message: err.message })
